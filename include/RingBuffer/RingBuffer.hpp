...
template <typename DataType, WaitPolicy WaitPolicy, size_t Capacity>
template <class AllocatorType>
void RingBuffer<DataType, WaitPolicy, Capacity>::allocate(
    AllocatorType& aAllocator) {
    if (is_allocated()) {
    ...
    mStorage = static_cast<std::byte*>(aAllocator.allocate(
        cNumBytes, sAlignment));
    if (mStorage == nullptr) {
    ...
    static_assert(sMaxWrap > 2,
                  "Capacity too large: integer wrap protection insufficient");
...
template <typename Datatype, WaitPolicy WaitPolicy, size_t Capacity>
template <class AllocatorType>
void RingBuffer<Datatype, WaitPolicy,
                Capacity>::free(AllocatorType& aAllocator) {
    if (!is_allocated()) {
        throw std::runtime_error("no memory to free...");
    }
    const int cPopIndex = mPopIndex.load(std::memory_order_relaxed);
    const int cPushIndex = mPushIndex.load(std::memory_order_relaxed);
    for (int i = cPopIndex; i != cPushIndex; i = bump_index(i)) {
        const int cStorageIndex = i % Capacity;
        std::byte* cAddress = mStorage + cStorageIndex * sizeof(Datatype);
        std::launder(reinterpret_cast<Datatype*>(cAddress))->~Datatype();
    }

    aAllocator.reset();
    mStorage = nullptr;
    mPushIndex.store(0, std::memory_order_relaxed);
    mPopIndex.store(0, std::memory_order_relaxed);
}
...
bool RingBuffer<Datatype, WaitPolicy, Capacity>::emplace(ArgTypes&&... aArgs) {
    ...
    if (const int cIndexDelta = cUnwrappedPushIndex - cUnwrappedPopIndex;
        cIndexDelta == Capacity || cIndexDelta == Capacity - mIndexEnd) {
        return false;
    }
...
template <typename Datatype, WaitPolicy WaitPolicy, size_t Capacity>
int RingBuffer<Datatype, WaitPolicy, Capacity>::bump_index(
...
template <typename Datatype, WaitPolicy WaitPolicy, size_t Capacity>
void RingBuffer<Datatype, WaitPolicy, Capacity>::increase_size(
    const int aNumPushed) {
    static constexpr auto sOrder = std::memory_order::relaxed;
    [[maybe_unused]] auto cPriorSize = mSize.fetch_add(aNumPushed, sOrder);
}

template <typename Datatype, WaitPolicy WaitPolicy, size_t Capacity>
void RingBuffer<Datatype, WaitPolicy, Capacity>::decrease_size(
    const int aNumPopped) {
    static constexpr auto sOrder = std::memory_order::relaxed;
