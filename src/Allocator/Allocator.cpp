...
void* Allocator::allocate(const int aAllocationSize, const int aAlignment)
{
    ...
    void* cAlignedPointer { std::align(aAlignment, aAllocationSize, mBufferOffestPtr, remainingSize) };
    if (cAlignedPointer != nullptr)
    {
        const unsigned long cAlignmentSize = reinterpret_cast<std::uintptr_t>(cAlignedPointer) - reinterpret_cast<std::uintptr_t>(mBufferOffestPtr);
        mUsedAmount += cAlignmentSize + aAllocationSize;
        mBufferOffestPtr = reinterpret_cast<void*>(aAllocationSize + reinterpret_cast<uintptr_t>(cAlignedPointer));
    }
